...
- **Audio Level Indicators** - Visual feedback- `file_sharing_module.py`: Implements the file sending and receiving logic, with support for:
  ...
  - File upload with progress indication and speed monitoring

### 📺 Screen & Slide Sharing  - File download with progress tracking

- **Single Presenter Mode** - One presenter at a time  - Real-time progress updates in the UI

- **Full Screen Capture** - Share entire screen  - Efficient chunked file transfer

- **High Quality** - Preserved clarity  - Automatic server-side file availability tracking

- **Presenter Lock** - Automatic conflict prevention

## Setup and Usage

### 💬 Group Text Chat

- **Real-time Messaging** - Instant delivery via TCP### Quick Start

- **Persistent History** - Full conversation log

- **Sender Identification** - Clear username display1. **Run the launcher script** that will set up the environment and install dependencies:

- **Reliable Delivery** - TCP ensures no message loss   ```bash

   python run.py

### 📁 File Sharing   ```

- **Secure Transfer** - TCP-based reliable transmission

- **Progress Tracking** - Real-time progress indicators2. **Enter Server Connection Details**:

- **Multi-file Support** - Share multiple files   - When the login dialog appears, enter:

- **Central Storage** - Server manages files     - Server IP address
...
## 💻 System Requirements3. **Enjoy the G-Meet Clone!**
...
- **CPU:** Intel Core i3 (2.0 GHz+)1. **Create and activate a virtual environment** (recommended):

- **RAM:** 4 GB   ```bash

- **Network:** 100 Mbps LAN   python -m venv venv
...
- **CPU:** Intel Core i5 (2.5 GHz+)   source venv/bin/activate

- **RAM:** 8 GB   ```

- **Network:** Gigabit (1000 Mbps) LAN

2. **Install Dependencies**:

---   ```bash

   pip install PyQt5 opencv-python pyaudio mss pillow

## 📦 Installation   ```



### Step 1: Install Python3. **Run the Server**:

Download Python 3.8+ from [python.org](https://www.python.org/downloads/)   - On one machine in the LAN, run the server script:

     ```bash

### Step 2: Create Virtual Environment     python server.py

     ```

**Windows (PowerShell):**   - The server will print a message indicating it has started.

```powershell

python -m venv .venv4. **Run the Client**:

.\.venv\Scripts\Activate.ps1   - On each client machine (including the server machine if you want to run a client there):
...
```bash   - When the login dialog appears, enter the server's IP address and your name.

python3 -m venv .venv

source .venv/bin/activate5. **Using the Application**:

```   - The main window shows a video grid in the center and panels on the right

   - Use the control buttons at the bottom to toggle your microphone, camera, and screen sharing

### Step 3: Install Dependencies   - The chat panel can be used for text communication

```bash   - The shared files panel displays all files available in the session

pip install -r requirements.txt   - To share a file, click the "Share File" button or use the "Share New File" button in the files panel

```   - To download a shared file, click the "Download" button next to the file in the files panel

   - All participants in the same session can see and download the shared files

**Required packages:**

- PyQt5 (GUI)## Future Improvements

- opencv-python (Video)

- pyaudio (Audio)- **Dynamic Grid Layout**: Improve the video grid to automatically resize and rearrange based on the number of participants.

- numpy (Processing)- **Audio Mixing**: Implement server-side audio mixing to reduce the number of audio streams sent to each client.

- Pillow (Images)- **Presenter Role**: Formalize the "presenter" role for screen sharing, so only one person can share at a time.

- mss (Screen capture)- **UI/UX Enhancements**: Improve the overall look and feel of the application.

- **Security**: Add encryption for all data transfers.

---- **Scalability**: Optimize the server to handle a larger number of clients more efficiently.
...
Server will display IP address (e.g., `192.168.1.100`)
